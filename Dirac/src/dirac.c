...
static int dirac_compare(const diminuto_tree_t * a, const diminuto_tree_t * b)
{
...
dirac_t * dirac_new(size_t rows, size_t columns)
{
    ...
    int rc = 0;
    diminuto_tree_t * you = diminuto_tree_search(root, &me, dirac_compare, &rc);
...
dirac_t * dirac_delete(dirac_t * that)
{
    ...
    diminuto_tree_t * you = diminuto_tree_search_insert_or_replace(&root, me, dirac_compare, !0);
